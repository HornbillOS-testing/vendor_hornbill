...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Fusion model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function deleteOTA() {
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request DELETE \
        https://api.fusion.org/api/ota/builds/$1/;
}
...
function pushOTA() {
    ...
    url="https://sourceforge.net/projects/fusionos-project/files/$device/${file##*/}";
    if [[ "${TEST_BUILD}" == true ]]; then
        url="https://sourceforge.net/projects/fusionos-project/files/$device/cucked/${file##*/}";
    fi
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request POST \
        --data "$data" \
        https://api.fusion.org/api/ota/builds/;
}
