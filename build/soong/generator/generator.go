...
func init() {
	android.RegisterModuleType("hornbill_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hornbillExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hornbillExpandVariables(ctx, dep_file)
	...
	cmd := hornbillExpandVariables(ctx, String(g.properties.Cmd))
