...
func init() {
	android.RegisterModuleType("fuse_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fuseExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fuseExpandVariables(ctx, dep_file)
	...
	cmd := fuseExpandVariables(ctx, String(g.properties.Cmd))
